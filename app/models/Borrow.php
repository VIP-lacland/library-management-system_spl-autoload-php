...
class Borrow {
    ...
    public function createLoanRequest($userId, $bookItemId) {
        // Ngày mượn là hôm nay, trạng thái pending
        $borrowDate = date('Y-m-d');
        
        $sql = "INSERT INTO Loans (user_id, book_items_id, borrow_date, status) 
                VALUES (:user_id, :item_id, :borrow_date, 'pending')";
        
        $stmt = $this->db->prepare($sql);
        return $stmt->execute(['user_id' => $userId, 'item_id' => $bookItemId, 'borrow_date' => $borrowDate]);
    }
